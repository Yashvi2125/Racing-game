...
function moveEnemy(myCar) {
    ...
    enemyCarList.forEach(function (enemyCar) {
        ...
        if (enemyCar.y > myCar.getBoundingClientRect().bottom) {
            player.score += 10;
            enemyCar.y = -300; // Reset the position of enemyCar to be passed again
        }
    });
}
...
function initializeGame() {
    ...
    player.start = true;
    player.speed = 5; // Reset speed to 5 at the start of the game
    player.score = 0;
    window.requestAnimationFrame(runGame);
